      ...
      <div className="relative hidden w-1/2 overflow-hidden lg:block">
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center px-12">
          ...
          <div className="flex items-center gap-3 rounded-full bg-white/5 px-5 py-2.5 backdrop-blur-sm ring-1 ring-white/10 animate-fade-in-up" style={{ animationDelay: '250ms' }}>
            ...
            <span className="text-sm font-medium text-slate-300">Funcional & Reactivo</span>
          </div>

          <div className="mt-12 max-w-sm space-y-3">
            {[
              { icon: <IconShield size={18} className="text-[#c5a255]" />, text: 'Seguridad OWASP Top 10 + ISO 27001' },
              { icon: <IconLive size={18} className="text-[#c5a255]" />, text: 'Datos en tiempo real — RxJS + WebSocket' },
              { icon: <IconBuilding size={18} className="text-[#c5a255]" />, text: 'Arquitectura Hexagonal + Saga Pattern' },
              { icon: <IconShield size={18} className="text-[#c5a255]" />, text: 'Backend Elixir/Phoenix — Inmutabilidad' },
          ...
          <div className="mt-12 flex items-center gap-6 text-[11px] text-slate-500 animate-fade-in-up" style={{ animationDelay: '600ms' }}>
            <div className="flex items-center gap-1.5"><IconLock size={12} /><span>Cifrado TLS 1.3</span></div>
            <div className="h-3 w-px bg-white/10" />
            <div className="flex items-center gap-1.5"><IconShield size={12} className="text-emerald-500/60" /><span>NIST 800-63B</span></div>
            <div className="h-3 w-px bg-white/10" />
            <span>Bcrypt 12R</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex w-full flex-col justify-center bg-gradient-to-b from-[#faf8f5] to-white px-6 lg:w-1/2 lg:px-16">
        <div className="mx-auto w-full max-w-md">
          <div className="mb-8 text-center lg:hidden">
            ...
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#c5a255]">Luxury Hotel & Spa</p>
          </div>
          ...
          {offline && (
            <div className="mb-4 flex items-center gap-2 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-700 ring-1 ring-amber-200 animate-fade-in">
              <IconOffline size={18} />
              <div><span className="font-semibold">Modo Demo</span> — Sin conexión al servidor, usando datos simulados</div>
            </div>
          )}
          ...
          {bloqueado && (
            <div className="mb-4 flex items-center gap-3 rounded-xl bg-red-50 px-4 py-4 ring-1 ring-red-200 animate-fade-in">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-red-600">Demasiados intentos fallidos. Espere <span className="font-bold">{tiempoBloqueo}s</span></p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="email" className="mb-1.5 block text-sm font-semibold text-slate-700">Correo electrónico</label>
            ...
            <button type="submit" disabled={loading || bloqueado}
              ...
              {loading ? (
                ...
                <span className="flex items-center justify-center gap-2"><IconLock size={16} /> Bloqueado ({tiempoBloqueo}s)</span>
              ) : 'Iniciar Sesión'}
            </button>
          </form>
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {[
                { label: 'Administrador', email: 'admin@hotelflux.com', pass: 'Admin123!', color: 'bg-purple-50 text-purple-700 ring-purple-200 hover:bg-purple-100', icon: <IconCrown size={16} /> },
                ...
                { label: 'Gerente', email: 'ana@hotelflux.com', pass: 'Gerente123!', color: 'bg-emerald-50 text-emerald-700 ring-emerald-200 hover:bg-emerald-100', icon: <IconShield size={16} /> },
              ].map((u) => (
          ...
          <div className="mt-8 flex items-center justify-center gap-4 border-t border-slate-200 pt-5">
            <div className="flex items-center gap-1.5 text-[11px] text-slate-400"><IconShield size={12} /><span>OWASP Top 10</span></div>
            <div className="h-3 w-px bg-slate-200" />
            <div className="flex items-center gap-1.5 text-[11px] text-slate-400"><IconLock size={12} /><span>ISO 27001</span></div>
            <div className="h-3 w-px bg-slate-200" />
