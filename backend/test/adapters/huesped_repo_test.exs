defmodule HotelFlux.Adapters.HuespedRepoTest do
  ...
  describe "obtener/1" do
    test "retorna huésped existente" do
      huesped = insertar_huesped()
      assert {:ok, encontrado} = HuespedRepo.obtener(huesped.id)
      assert encontrado.id == huesped.id
    end
  ...
  describe "crear/1" do
    ...
    test "falla sin nombre requerido" do
      attrs = %{apellido: "Sin nombre", email: email_unico()}
      assert {:error, changeset} = HuespedRepo.crear(attrs)
      assert Keyword.has_key?(changeset.errors, :nombre)
    end

    test "falla con email duplicado" do
      email = email_unico()
      {:ok, _} = HuespedRepo.crear(%{nombre: "A", apellido: "B", email: email})
      assert {:error, changeset} = HuespedRepo.crear(%{nombre: "C", apellido: "D", email: email})
      assert Keyword.has_key?(changeset.errors, :email)
    end

    test "falla con email malformado" do
      attrs = %{nombre: "Ana", apellido: "Sol", email: "noesmail"}
      assert {:error, changeset} = HuespedRepo.crear(attrs)
      assert Keyword.has_key?(changeset.errors, :email)
    end
  end
